...
class AuthServiceImplement extends ServiceApi implements AuthService
{
     ...
     * assigns the USER role, and sends email verification notification.
     *
     * @param  mixed  $request  The validated form request containing name, email, and password
    ...
    public function register($request): AuthServiceImplement
    {
        try {
            ...
            $user = $this->mainRepository->createUser($request->only(['first_name', 'last_name', 'email', 'password']));
     ...
     *
     * Validates user credentials, revokes existing tokens,
     * and generates a new authentication token.
     *
     ...
     *
     * Sends a password reset notification to the user's email address.
     *
     * @param  mixed  $request  The validated form request containing email
     * @return AuthServiceImplement Returns service response indicating password reset link sent status
     */
    public function forgotPassword($request): AuthServiceImplement
    {
        try {
            ...
            return $this->setCode(ResponseCode::BAD_REQUEST->value)
                ->setMessage('Unable to send password reset link. Please try again.');
        } catch (\Exception $e) {
            return $this->setCode(ResponseCode::SERVER_ERROR->value)
     ...
     *
     * @param  mixed  $request  The validated form request containing token, email, and new password
     * @return AuthServiceImplement Returns service response indicating password reset status
     */
